  ...
  return <div className="space-y-6">
    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        {loading ? <p className="py-10 text-center text-sm text-slate">Cargando inversiones...</p> : !selectedId ? <Card className="p-[10px]"><p className="text-sm text-slate">Crea una inversión en el maestro para comenzar.</p></Card> : summary && <><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="p-[10px]"><p className="text-xs text-slate">Capital neto</p><p className="mt-1 font-display text-xl font-semibold">{money(summary.netCapital, summary.currency)}</p></Card>
          <Card className="p-[10px]"><p className="text-xs text-slate">Valor actual</p><p className="mt-1 font-display text-xl font-semibold">{money(summary.currentValue, summary.currency)}</p></Card>
          <Card className="p-[10px]"><p className="text-xs text-slate">Ganancia total</p><p className="mt-1 font-display text-xl font-semibold">{money(summary.totalGain, summary.currency)}</p></Card>
          <Card className="p-[10px]"><p className="text-xs text-slate">Rentabilidad</p><p className="mt-1 flex items-center gap-1 font-display text-xl font-semibold"><TrendingUp size={18}/>{summary.returnPct?.toFixed(2) ?? '0.00'}%</p></Card>
        </div>
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="p-[10px]"><h3 className="font-display text-lg font-semibold">Resumen</h3><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate">Aportes</dt><dd className="font-medium">{money(summary.contributions, summary.currency)}</dd></div><div><dt className="text-slate">Retiros</dt><dd className="font-medium">{money(summary.withdrawals, summary.currency)}</dd></div><div><dt className="text-slate">Ganancia realizada</dt><dd className="font-medium">{money(summary.realizedGain, summary.currency)}</dd></div><div><dt className="text-slate">Ganancia no realizada</dt><dd className="font-medium">{money(summary.unrealizedGain, summary.currency)}</dd></div><div><dt className="text-slate">Intereses/dividendos</dt><dd className="font-medium">{money(summary.income, summary.currency)}</dd></div><div><dt className="text-slate">Comisiones</dt><dd className="font-medium">{money(summary.fees, summary.currency)}</dd></div></dl></Card>
